...
def fractalGeneration(width, height, zoom, moveX, moveY, cX, cY, maxIter):
    ...
    for x in range(width):
        if x%20 == 0:
            percentageProgress = round((x/width) * 100)
            headerBar.set_subtitle("Rendering progress: " + str(percentageProgress) + "%")
...
class Handler:
    ...
    def executeButtonPressed(self, button):
        headerBar.set_subtitle("Setting up...")
        progressThrobber.start()

        time.sleep(0.5)

        processThread = threading.Thread(target = fractalGeneration, args = (
